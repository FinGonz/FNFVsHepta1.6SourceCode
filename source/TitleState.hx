...
class TitleState extends MusicBeatState
{
	...
	[
		'backgrounds/titlescreen',
		'backgrounds/titlescreen2',
		'backgrounds/titlescreen3',
	];
	...
	function startIntro()
	{
		...
		bgScreen = new FlxSprite(0).loadGraphic(randomizeBG());
		bgScreen.screenCenter();
		add(bgScreen);
		...
		swagShader = new ColorSwap();
			gfDance = new FlxSprite(titleJSON.gfx, -1500);
			gfDance.scale.set(titleJSON.gfscale, titleJSON.gfscale);
		...
		titleText = new FlxSprite(1500, titleJSON.starty);
		#if (desktop && MODS_ALLOWED)
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxTween.tween(titleText, {x: -1500}, 1.4, {ease: FlxEase.expoInOut});
				FlxTween.tween(gfDance, {y: 1500}, 1.4, {ease: FlxEase.expoInOut});
		...
		if(gfDance != null) {
			...
			if(!closedState) {
				...
				switch (sickBeats)
				{
					...
					case 2:
						createCoolText(['Too many people to list here'], 120);
					case 3:
						createCoolText(['Check the GB or GJ Page'], 180);
					case 4:
						deleteCoolText();
					case 5:
					...
					// credTextShit.text += '\nVs';
					case 13:
						addMoreText('Hepta'); // credTextShit.text += '\nHepta';
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxTween.tween(titleText, {x: 100}, 1.4, {ease: FlxEase.expoInOut});
			FlxTween.tween(gfDance, {y: 30}, 1.4, {ease: FlxEase.expoInOut});
