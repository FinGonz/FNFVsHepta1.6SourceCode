...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['dawgfuck', 0.2], //From 0% to 19%
		['piece of shit', 0.4], //From 20% to 39%
		['insert funky friday reference', 0.5], //From 40% to 49%
		['bit sad innit bruv', 0.6], //From 50% to 59%
		['mmmeh', 0.69], //From 60% to 68%
		['noice', 0.7], //69%
		['le good', 0.8], //From 70% to 79%
		['great or something', 0.9], //From 80% to 89%
		['sick!!!!', 1], //From 90% to 99%
		['hecker', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	private var camFollowClone:FlxPoint;
	...
	public var cameraSpeedClone:Float = 1;
	...
	var dodgeTimer1:FlxTimer;
	var dodgeTimer2:FlxTimer;
	...
	private var lmaoPart:Bool = false;

	var black:FlxSprite;

	var mikoCrowd:BGSprite;
	var topMikoCrowd:BGSprite;
	var mikoCrowdFront:BGSprite;
	var mikoDay:BGSprite;
	var mikoMidday:BGSprite;
	var mikoNight:BGSprite;
	...
	var funnyFloatyBoys:Array<String> = ['guest666'];
	...
	override public function create()
	{
		...
		redGlow = new FlxSprite(-120, -120).loadGraphic(Paths.image('hepta/GlowRed'));
		...
		spacebar = new FlxSprite(0, 0).loadGraphic(Paths.image('hepta/spacebar'));
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			...
			{
				case 'seven-sides':
					curStage = 'miko-borgar';
				case 'heptalogy':
					curStage = 'miko-borgar-midday';
				case 'heptagonistic':
					curStage = 'miko-borgar-night';
				case 'vandalize' | 'kaboom':
					curStage = 'miko-borgar';
				case 'unknown-suffering':
					curStage = 'miko-borgar-night-unknown';
				default:
					curStage = 'miko-borgar';
			}
		}
		...
		switch (curStage)
		{
			...
			case 'schoolEvil': //Week 6 - Thorns
				...
				case 'miko-borgar': //WeekHepta
			    {
				mikoDay = new BGSprite('hepta/miko-borgar', -700, -500, 0.9, 0.9);
				add(mikoDay);

				mikoMidday = new BGSprite('hepta/miko-borgar-midday', -700, -500, 0.9, 0.9);
				mikoMidday.visible = false;
				add(mikoMidday);

				mikoNight = new BGSprite('hepta/miko-borgar-night', -700, -500, 0.9, 0.9);
				mikoNight.visible = false;
				add(mikoNight);
		        }

			case 'miko-borgar-midday': //WeekHepta
			    {
					mikoMidday = new BGSprite('hepta/miko-borgar-midday', -700, -500, 0.9, 0.9);
					add(mikoMidday);
			...
			case 'miko-borgar-night': //WeekHepta
			    {
					mikoNight = new BGSprite('hepta/miko-borgar-night', -700, -500, 0.9, 0.9);
					add(mikoNight);
			...
			case 'miko-borgar-night-unknown': //covers since ye
				{
					mikoNight = new BGSprite('hepta/miko-borgar-night', -700, -500, 0.9, 0.9);
					add(mikoNight);
				}
		...
		if(curStage == 'miko-borgar-night')
			...
			mikoCrowdFront.setGraphicSize(Std.int(mikoCrowdFront.width * 2.11));
				mikoCrowdFront.updateHitbox();
					mikoCrowdFront.antialiasing = true;
					add(mikoCrowdFront);
			}
		...
		if(gfVersion == null || gfVersion.length < 1) {
			switch (curStage)
			{
				case 'miko-borgar' | 'miko-borgar-midday' | 'miko-borgar-night':
					gfVersion = 'gf-miko';
				default:
					gfVersion = 'gf';
			}
		...
		var file:String = Paths.json(songName + '/dialogue-end'); //Checks for End json/Psych Engine End dialogue
		if (OpenFlAssets.exists(file)) {
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'vandalize' | 'kaboom':
				credits = 'Original Song by BBPanzu! Cover by Izzat Radzif!';
			case 'unknown-suffering':
				credits = 'Song from Wednesdays Infidelity. Cover by Gemtin.';
			case 'heptas-spam-challenge':
				credits = 'Fuck you. This is what you get for cheating.';
			default:
				credits = '';
		}

		var creditsText:Bool = credits != '';
		var textYPos:Float = healthBarBG.y + 32;
		if (creditsText)
		{
			textYPos = healthBarBG.y + 32;
		}

		// Add Psych Engine watermark
		psychEngineWatermark = new FlxText(4,healthBarBG.y + 52,0,(Main.watermarks ? "Psych Engine: Vs Hepta " + MainMenuState.vsHeptaVersion : ""), 16);
		...
		psychEngineWatermark.visible = !ClientPrefs.hideHud;
		psychEngineWatermark.visible = !ClientPrefs.hideWatermark;
		add(psychEngineWatermark);

		if (ClientPrefs.downScroll)
			psychEngineWatermark.y = FlxG.height * 0.9 + 47;

		creditsWatermark = new FlxText(4, healthBarBG.y + 32, 0, credits, 16);
		...
		creditsWatermark.visible = !ClientPrefs.hideWatermark;
		add(creditsWatermark);

		if (ClientPrefs.downScroll)
			creditsWatermark.y = FlxG.height * 0.9 + 27;
		...
		judgementCounter.text = 'Summary:\n\nSicks: ${sicks}\nGoods: ${goods}\nOks: ${bads}\nBads: ${shits}\nMisses: ${songMisses}';
		judgementCounter.visible = ClientPrefs.hideHud;
		judgementCounter.visible = ClientPrefs.judgement;
		...
		if (isStoryMode && !seenCutscene || ClientPrefs.freeplayDialogue)
		{
			switch (daSong)
			{
				case "monster":
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					case 'seven-sides':
						startDialogue(dialogueJson, 'seven-sides-dialogue');
					case 'heptalogy':
						startDialogue(dialogueJson, 'heptalogy-dialogue');
					case "heptagonistic":
						startDialogue(dialogueJson, 'heptagonistic-dialogue');
					case "vandalize":
						startDialogue(dialogueJson, 'vandalize-dialogue');
					case "kaboom":
						startDialogue(dialogueJson, 'kaboom-dialogue');
					case "unknown-suffering":
						startDialogue(dialogueJson, 'unknown-dialogue');
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(curStage == 'miko-borgar-midday') {
						mikoCrowd.dance(true);
				}
				
				if(curStage == 'miko-borgar-night') {
						mikoCrowd.dance(true);

					mikoCrowdFront.dance(true);
				}
	...
	function startSong():Void
	{
		...
		FlxTween.tween(laneunderlayOpponent, {alpha: ClientPrefs.laneTransparency}, 0.5, {ease: FlxEase.quadOut});
        FlxTween.tween(laneunderlay, {alpha: ClientPrefs.laneTransparency}, 0.5, {ease: FlxEase.quadOut});
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curSong.toLowerCase())
		{
			...
			case 'heptas-spam-challenge':
				ClientPrefs.ghostTapping = false;
			if (FlxG.keys.anyJustPressed(debugKeysChart) && !endingSong && !inCutscene || FlxG.keys.justPressed.F1) {
				System.exit(0);
				FlxG.save.data.spamFound = true;
			}
			case 'vandalize':
				gf.visible = false;
		}
	...
	if (SONG.song.toLowerCase() == 'heptagonistic')
		    {
		...
		if (SONG.song.toLowerCase() == 'heptas-spam-challenge')
			{
				playerStrums.forEach(function(spr:FlxSprite)
				{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Accuracy: ' + ratingName + ' | Combo: ' + combo;
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Accuracy: ' +  Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ' - ' + ratingFC + ' | Combo: ' + combo;
		}
		...
		if (FlxG.keys.anyJustPressed(debugKeysChart) && !endingSong && !inCutscene || FlxG.keys.justPressed.F1)//basically when you press 7 or F1
		{
			cancelFadeTween();
			CustomFadeTransition.nextCamera = camOther;
			...
			persistentUpdate = false;
			paused = true;
			PlayState.SONG = Song.loadFromJson("heptas-spam-challenge", "heptas-spam-challenge"); // fuck you for cheating!
		...
		/*if (dad.curCharacter == "guest666"){
			dad.y += Math.sin(floatshit);
		}
		
		var curSection:Int = Math.floor(curStep / 16); old code :D
			if (SONG.notes[curSection] != null)
			{
			isCameraOnForcedPos = false;
		if(!SONG.notes[curSection].mustHitSection && ClientPrefs.camMove) {
			//very messy but i dont give a shit since it works :D
			...
			if (dad.animation.curAnim.name.endsWith("DOWN-alt")){
				 camFollow.x = 700;
				 camFollow.y = 560;
				 isCameraOnForcedPos = true;
			}
			if (dad.animation.curAnim.name.endsWith("UP-alt")){
				 camFollow.x = 700;
					camFollow.y = 440;
					isCameraOnForcedPos = true;
			}
			if (dad.animation.curAnim.name.endsWith("RIGHT-alt")){
				 camFollow.x = 760;
					camFollow.y = 500;
					isCameraOnForcedPos = true;
			}
			if (dad.animation.curAnim.name.endsWith("LEFT-alt")){
				 camFollow.x = 640;
					camFollow.y = 500;
					isCameraOnForcedPos = true;
			}
		...
		if(SONG.notes[curSection].mustHitSection && ClientPrefs.camMove) {
			...
			if (boyfriend.animation.curAnim.name.endsWith("UP")){
				 camFollow.x = 1100;
				 camFollow.y = 440;
				 isCameraOnForcedPos = true;
			}
			...
			if (boyfriend.animation.curAnim.name.endsWith("LEFT")){
				 camFollow.x = 1040;
				 camFollow.y = 500;
				 isCameraOnForcedPos = true;
			}
			if (boyfriend.animation.curAnim.name.endsWith("DOWN-alt")){
				 camFollow.x = 1100;
				 camFollow.y = 560;
				 isCameraOnForcedPos = true;
			}
			if (boyfriend.animation.curAnim.name.endsWith("UP-alt")){
				 camFollow.x = 1100;
				 camFollow.y = 440;
				 isCameraOnForcedPos = true;
			}
			if (boyfriend.animation.curAnim.name.endsWith("RIGHT-alt")){
				 camFollow.x = 1160;
				 camFollow.y = 500;
				 isCameraOnForcedPos = true;
			}
			if (boyfriend.animation.curAnim.name.endsWith("LEFT-alt")){
				 camFollow.x = 1040;
				 camFollow.y = 500;
				 isCameraOnForcedPos = true;
			}
			if (boyfriend.animation.curAnim.name.endsWith("ge")){
				 camFollow.x = 1100;
				 camFollow.y = 500;
				 isCameraOnForcedPos = true;
			}
			if (boyfriend.animation.curAnim.name.endsWith("rt")){
				 camFollow.x = 1100;
				 camFollow.y = 500;
				 isCameraOnForcedPos = true;
			}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'White Flash':
				...
				{
				var val1:Float = Std.parseFloat(value1);
				FlxG.camera.flash(FlxColor.WHITE, val1);
				}
	...
	public function endSong():Void
	{
		var daSong:String = Paths.formatToSongPath(curSong);
				...
				{
						case 'heptagonistic':
							if (isStoryMode && !seenCutscene && endingSong){
							startDialogue(dialogueJsonEnd, 'heptagonistic-dialogue');
							}
						case 'heptas-spam-challenge':
							PlayState.SONG = Song.loadFromJson("heptas-spam-challenge", "heptas-spam-challenge"); // fuck you for cheating!
							cancelFadeTween();
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['heptagon_destroyer', 'ur_bad',
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(daNote.noteType == 'Scythe Note') {
			if(boyfriend.animOffsets.exists('hurt')) {
				boyfriend.playAnim('hurt', true);
				boyfriend.specialAnim = true;
			}
			if(dad.animOffsets.exists('throw')) {
	...
	function shakewindow() // from definitive edition opposition
		{
			new FlxTimer().start(0.01, function(tmr:FlxTimer)
			{
				Lib.application.window.move(Lib.application.window.x + FlxG.random.int( -20, 20),Lib.application.window.y + FlxG.random.int( -20, 20));
			}, 20);
		}
...
// made the health drain character dependent now
		if(SONG.player2 == 'hepta' && health > 0.1)
			{
			health -= 0.04;
			}
		
		if(SONG.player2 == 'cb' && health > 0.1)
			{
			health -= 0.07;
			}
		
		if(SONG.player2 == 'guest' && health > 0.1)
			{
			health -= 0.06;
			}
		
		if(SONG.player2 == 'guest666' && health > 0.1)
			{
			health -= 0.09;
			}
		
		if(SONG.player2 == 'izzat' && health > 0.1)
			{
			health -= 0.02;
			}
		
		if(SONG.player2 == 'rookie' && health > 0.1)
			{
			health -= 0.01;
			}
                    ...
                    if (!dad.stunned)
                        {
							...
							switch(Std.int(Math.abs(note.noteData)))
							{
								case 2:
                        	        ...
                        	        if(ClientPrefs.camMove && dad.animation.curAnim.name != 'singUP')
									{ 
										camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
			            	            camFollow.x += dad.cameraPosition[0];
			            	            camFollow.y += dad.cameraPosition[1];                       
								...
								case 3:							
                        	        ...
                        	        if(ClientPrefs.camMove && dad.animation.curAnim.name != 'singRIGHT')
										{
											camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
			            	        		camFollow.x += dad.cameraPosition[0];
			            	        		camFollow.y += dad.cameraPosition[1];
								...
								case 1:
                        	        ...
                        	        if(ClientPrefs.camMove && dad.animation.curAnim.name != 'singDOWN')
									{
										camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
			            	            camFollow.x += dad.cameraPosition[0];
			            	            camFollow.y += dad.cameraPosition[1];
								...
								case 0:
			            	        ...
			            	        camFollow.y += dad.cameraPosition[1];
                        	   		...
                        	   		if(ClientPrefs.camMove && dad.animation.curAnim.name != 'singLEFT')
									{
										camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
			            	    	    camFollow.x += dad.cameraPosition[0];
			            	    	    camFollow.y += dad.cameraPosition[1];
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			if(SONG.player1 == 'izzat-playable')
				{
				health += 0.015;
				}
				if(ClientPrefs.hitsounds && !note.isSustainNote  && !note.ignoreNote && !note.hitCausesMiss)
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Scythe Note': //Scythe Note, This is only used for Vandalize
						boyfriend.playAnim('dodge', true);
						boyfriend.specialAnim = true;
	...
	override function beatHit()
	{
		...
		switch (SONG.song.toLowerCase()) //using hardcoded shit instead of fuckall events since its easier
			{
				case 'kaboom':
					switch (curStep)
				{
					...
					case 512:
						if(ClientPrefs.flashing) FlxG.camera.flash(FlxColor.WHITE, 1);
						mikoDay.visible = false;
						mikoMidday.visible = true;
					case 1024:
						if(ClientPrefs.flashing) FlxG.camera.flash(FlxColor.WHITE, 1);
					case 1280:
						if(ClientPrefs.flashing) FlxG.camera.flash(FlxColor.WHITE, 1);
						mikoMidday.visible = false;
						mikoNight.visible = true;
						redGlow.visible = true;
						creditsWatermark.text = 'Oh God... Cover by Izzat Radzif...';
					case 1536:
						if(ClientPrefs.flashing) FlxG.camera.flash(FlxColor.WHITE, 1);
						redGlow.visible = false;
				}
		...
		switch (curStage)
		{
			...
			case "philly":
				...
				case "miko-borgar-midday":
					mikoCrowd.dance(true);
				
				case "miko-borgar-night":
					mikoCrowd.dance(true);
					mikoCrowdFront.dance(true);
		}
				...
				switch(achievementName)
				{
					case 'heptagon_destroyer':
						if(isStoryMode && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'weekHepta':
									if(achievementName == 'heptagon_destroyer') unlock = true;
							}
						}
					case 'ur_bad':
